...
if testcase "basic"; then
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log FAILED >"${_builddir}/fail-once.log"
	...
	_builddir="${TSHDIR}/arm64/2022-10-25.1"
	_time="${_2022_10_25}"
	mkbuilddir "${_builddir}"
	{
		step_serialize -s 1 -n test/pass -l pass.log -t "${_time}"
	...
	_builddir="${TSHDIR}/arm64/2022-10-24.1"
	_time="${_2022_10_24}"
	mkbuilddir "${_builddir}"
	{
		step_serialize -s 1 -n test/pass -l pass.log -t "${_time}"
...
if testcase "patches"; then
	...
	_time="${_2022_10_25}"
	mkbuilddir "${_builddir}"
	printf 'src.diff.1\n' >"${_builddir}/src.diff.1"
	printf 'src.diff.2\n' >"${_builddir}/src.diff.2"
	{
		step_serialize -s 1 -n test/pass -l pass.log -t "${_time}"
		step_log PASSED >"${_builddir}/pass.log"
...
if testcase "multiple invocations per day"; then
	_builddir="${TSHDIR}/amd64/2022-10-25.2"
...
if testcase "non-regress suites"; then
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log PASSED >"${_builddir}/first.log"

		step_serialize -H -s 2 -n ../b -l b.log -t "${_time}"
		step_log PASSED >"${_builddir}/b.log"
...
if testcase "missing runs"; then
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log PASSED >"${_builddir}/always.log"
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log SKIPPED >"${_builddir}/once.log"

		step_serialize -H -s 2 -n end -t "$((_time + 3600))"
	} >"$(step_path "${_builddir}")"

	_builddir="${TSHDIR}/amd64/2022-10-23.1"
	_time="${_2022_10_23}"
	mkbuilddir "${_builddir}"
	{
		step_serialize -s 1 -n test/always -l always.log -t "${_time}"
...
if testcase "suites ordering"; then
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log PASSED >"${_builddir}/new.log"
...
if testcase "unexpected pass"; then
	...
	mkbuilddir "${_builddir}"
	{
		...
		step_log FAILED >"${_builddir}/fail.log"
...
if testcase "failed and skipped"; then
	...
	{
		step_serialize -s 1 -n test/fail -l fail.log -t "${_time}" -e 1
		{ step_log FAILED; step_log SKIPPED; } >"${_builddir}/fail.log"
