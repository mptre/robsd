if testcase "basic"; then
	if ! report_skip -n "end" -l "/dev/null"; then
		fail "expected end to be skipped"
	fi
	if report_skip -n "cvs" -l "/dev/null"; then
...
if testcase "checkflist empty"; then
	...
	if ! report_skip -n "checkflist" -l "$TMP1"; then
...
if testcase "diff src present"; then
	...
	if report_skip -n "patch" -l "/dev/null"; then
		fail "expected patch to not be skipped"
	fi
	if report_skip -n "revert" -l "/dev/null"; then
...
if testcase "regress"; then
	if ! (setmode "robsd-regress" &&
		report_skip -n "bin/cat" -l "/dev/null")
	then
...
if testcase "regress skip"; then
	...
	if ! (setmode "robsd-regress" &&
		SKIPIGNORE="bin/cat" report_skip -n "bin/cat" -l "$TMP1")
	then
		fail "expected step to be skipped"
	fi
fi
