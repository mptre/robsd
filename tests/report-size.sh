...
if testcase "previous release missing"; then
	assert_eq "" "$(report_size -b "$_builddir" -r "$ROBSDDIR" "$TMP1")"
fi
...
if testcase "previous delta too small"; then
	...
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}/rel
	zero 1K "${ROBSDDIR}/2019-02-22/rel/${_file}"
...
if testcase "previous delta megabytes"; then
	...
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}/rel
	zero "$((1024 * 1024))" "${ROBSDDIR}/2019-02-22/rel/${_file}"

	assert_eq "${_file} 1.4M (+428.6K)" \
		"$(report_size -b "$_builddir" -r "$ROBSDDIR" "$TMP1")"
fi
...
if testcase "previous delta negative"; then
	...
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}/rel
	zero 2M "${ROBSDDIR}/2019-02-22/rel/${_file}"

	assert_eq "${_file} 1.0K (-2.0M)" \
		"$(report_size -b "$_builddir" -r "$ROBSDDIR" "$TMP1")"
fi
