...
if testcase "basic"; then
	...
	step_serialize -n end -d 1800 >"${ROBSDDIR}/2019-02-22/steps"
...
if testcase "delta negative"; then
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}
	step_serialize -n end -d 3600 >"${ROBSDDIR}/2019-02-22/steps"
...
if testcase "delta below threshold"; then
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}
	step_serialize -n end -d 30 >"${ROBSDDIR}/2019-02-22/steps"
...
if testcase "previous build failed"; then
	# shellcheck disable=SC2086
	mkdir -p ${ROBSDDIR}/2019-02-{22,23}
	step_serialize -n kernel -e 1 -d 3600 >"${ROBSDDIR}/2019-02-22/steps"
...
if testcase "previous failed and successful"; then
	...
	step_serialize -n kernel -e 1 -d 3600 >"${ROBSDDIR}/2019-02-22/steps"
	step_serialize -n end -d 3600 >"${ROBSDDIR}/2019-02-21/steps"
